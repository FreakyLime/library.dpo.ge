...
class MoreImagesController extends MainController
{
    ...
    public function SaveMoreImages(Request $request, MoreImages $MoreImages, ImageLybrary $imageLybrary){
        ...
        return Redirect("admin/".$More_images_forms['module']."_module/edit_".$More_images_forms['module']."/".$More_images_forms['record_item_id']."#images");
    }

    public function DeleteImage($table, $item, $img_id, MoreImages $moreImages, NewsModule $NewsModule,ImageLybrary $imageLybrary){

        /*ინფორმაციის წაშლა + სურათის სახელის აღება*/
        $MoreImage = $moreImages->DeleteMoreImages($img_id);

        //სურათის წაშლა ფოლდერიდან
        $imageLybrary->DeleteImage($MoreImage['image_name'],$MoreImage['tumb']);

        return Redirect("admin/".$table."_module/edit_".$table."/".$item."#images");
    }
