            ...
            if (!$file->isDot() && !$this->exceptExtensions->contains($file->getExtension()) && !$this->exceptFolders->contains($file->getBasename()) && !$this->exceptFiles->contains($file->getBasename())  && $this->accept($file)) {
                ...
                if($file->isReadable()){
                    $fileInfo = [
                        ...
                        'thumb' =>  asset('/public/'.$this->getThumb($file, 'files/file_manager/photos/1')),
                        //'thumb' =>  asset('/public/'.$this->getThumb($file, $this->publicPath)), *tumb
                        //'thumb' =>  asset('/public/'.$this->publicPath.'/'.$file->getBasename()), *image
