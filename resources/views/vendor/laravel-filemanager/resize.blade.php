  ...
  <div class="col-md-4">
    ...
    <table class="table table-compact table-striped">
      ...
      <tbody>
        @if ($scaled)
        <tr>
          <td>{{ trans('laravel-filemanager.resize-ratio') }}</td>
          <td>{{ number_format($ratio, 2) }}</td>
        </tr>
        <tr>
          <td>{{ trans('laravel-filemanager.resize-scaled') }}</td>
          <td>
            {{ trans('laravel-filemanager.resize-true') }}
          </td>
        </tr>
        @endif
        <tr>
          <td>{{ trans('laravel-filemanager.resize-old-height') }}</td>
          <td>{{ $original_height }}px</td>
        </tr>
        <tr>
          <td>{{ trans('laravel-filemanager.resize-old-width') }}</td>
          <td>{{ $original_width }}px</td>
        </tr>
        <tr>
          <td>{{ trans('laravel-filemanager.resize-new-height') }}</td>
          <td><span id="height_display"></span></td>
        </tr>
        <tr>
          <td>{{ trans('laravel-filemanager.resize-new-width') }}</td>
          <td><span id="width_display"></span></td>
        </tr>
      </tbody>
    </table>

    <button class="btn btn-primary" onclick="doResize()">{{ trans('laravel-filemanager.btn-resize') }}</button>
    <button class="btn btn-info" onclick="loadItems()">{{ trans('laravel-filemanager.btn-cancel') }}</button>
