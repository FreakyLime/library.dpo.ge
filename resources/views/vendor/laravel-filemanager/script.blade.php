...
$(document).ready(function () {
  bootbox.setDefaults({locale:"{{ Lang::get('laravel-filemanager.locale-bootbox') }}"});
  // load folders
  loadFolders();
  loadItems();
  setOpenFolders();
...
$('#add-folder').click(function () {
  bootbox.prompt("{{ Lang::get('laravel-filemanager.message-name') }}", function (result) {
    if (result !== null) {
      createFolder(result);
    }
  });
...
$('#upload-btn').click(function () {
  ...
  function showRequest(formData, jqForm, options) {
    $('#upload-btn').html('<i class="fa fa-refresh fa-spin"></i> {{ Lang::get("laravel-filemanager.btn-uploading") }}');
    return true;
  }

  function showResponse(responseText, statusText, xhr, $form)  {
    $('#uploadModal').modal('hide');
    $('#upload-btn').html('{{ Lang::get("laravel-filemanager.btn-upload") }}');
    if (responseText != 'OK'){
      notify(responseText);
    }
    $('input#upload').val('');
  ...
  function showError(jqXHR, textStatus, errorThrown) {
    $('#upload-btn').html('{{ Lang::get("laravel-filemanager.btn-upload") }}');
    if (jqXHR.status == 413) {
      notify('{{ Lang::get("laravel-filemanager.error-too-large") }}');
    } else if (textStatus == 'error') {
      notify('{{ Lang::get("laravel-filemanager.error-other") }}' + errorThrown);
    } else {
      notify('{{ Lang::get("laravel-filemanager.error-other") }}' + textStatus + '<br>' + errorThrown);
    }
  }
...
function rename(item_name) {
  bootbox.prompt({
    title: "{{ Lang::get('laravel-filemanager.message-rename') }}",
    value: item_name,
...
function trash(item_name) {
  bootbox.confirm("{{ Lang::get('laravel-filemanager.message-delete') }}", function (result) {
    if (result == true) {
      $.ajax({
        type: 'GET',
        dataType: 'text',
