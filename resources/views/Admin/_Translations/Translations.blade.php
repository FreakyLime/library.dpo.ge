...
<div style="width: 100%; margin: auto;">
    <h3 class="bpg_caps fbold">ვებ-გვერდის სიტყვების თარგმანი</h3>
    <p>ყურადღება ნათარგმნი სიტყვები არ გამოჩნდება ვებ-გვერდზე სანამ არ დააჭერთ ღილაკს გამოქვეყნება***</p>
    ...
    <form class="form-inline form-import" method="POST" action="<?= action('\Barryvdh\TranslationManager\Controller@postImport') ?>" data-remote="true" role="form">
        ...
        <select name="replace" class="form-control">
            <option value="0">ახალი სიტყვების მოძებნა</option>
            <option value="1">არსებული სიტყვების განახლება</option>
        </select>
        <button type="submit" class="btn btn-success"  data-disable-with="Loading..">ჯგუფების შემოტანა</button>
    </form>
    <form class="form-inline form-find" method="POST" action="<?= action('\Barryvdh\TranslationManager\Controller@postFind') ?>" data-remote="true" role="form" data-confirm="გსურთ ენების ფოლდერის დასკანირება? ყვენა ნაპოვნი სიტყვა ჩაიწერება მონაცემთა ბაზაში">
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        <p></p>
        <button type="submit" class="btn btn-info" data-disable-with="Searching.." >სიტყვების მოძებნა ფაილებში</button>
    </form>
    <?php endif; ?>
    <?php if(isset($group)) : ?>
    <form class="form-inline form-publish" method="POST" action="<?= action('\Barryvdh\TranslationManager\Controller@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="გსურთ ნათარგმნი სიტყვების ჯგუფის გამოქვეყნება '<?= $group ?>?">
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        <button type="submit" class="btn btn-info bpg_caps fbold" data-disable-with="დაელოდეთ გამოქვეყნებას.." >გამოქვეყნება</button>
        <a href="<?= action('\Barryvdh\TranslationManager\Controller@getIndex') ?>" class="btn btn-default bpg_caps fbold">უკან</a>
    </form>
    <?php endif; ?>
    </p>
    <form role="form">
    ...
    <form action="<?= action('\Barryvdh\TranslationManager\Controller@postAdd', array($group)) ?>" method="POST"  role="form">
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        <textarea class="form-control" rows="3" name="keys" placeholder="ახალი სიტყვის დამატება"></textarea>
        <p></p>
        <input type="submit" value="სიტყვის დამატება" class="btn btn-primary">
    </form>
    <hr>
    <h4 class="bpg_caps fbold">სულ: <?= $numTranslations ?>, განახლებული: <?= $numChanged ?></h4>
    <table class="table">
        <thead>
        <tr>
    ...
    <?php else: ?>
    <p>აირჩიეთ ჯგუფი რომ თარგმნოთ სიტყვები***</p>
